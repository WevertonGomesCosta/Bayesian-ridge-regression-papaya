...
data.frame(
  Item = c(
    ...
    "Traits with supported predictive evidence"
  ),
  Value = c(
    length(traits),
    length(methods),
...
final_results_summary <- model_selection |>
  mutate(
    ...
    Decision_label = case_when(
      ...
      Decision_code == "rrblup_preferred_among_equivalent_models" ~
        "RR-BLUP preferred among equivalent methods",
      Decision_code == "best_non_rrblup_equivalent_model" ~
    ...
    Selection_status_label = case_when(
      Selection_status == "automatic_supported" ~
...
Selection uses traits with positive OOF correlation and `p < 0.05`. The
proportion used is `r sprintf("%.0f%%", 100 * selection_proportion)` of the
individuals within each trait, respecting the breeding direction defined in
Module 04.
...
Recurrence summarizes how many supported traits place each individual among the
