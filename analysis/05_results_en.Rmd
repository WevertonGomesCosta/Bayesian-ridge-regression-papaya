...
# Build `method_labels` with the information required by the next operation.
method_labels <- c(
  RRBLUP = "RR-BLUP/BRR",
  BayesA = "BayesA",
  BayesB = "BayesB",
...
# Build `method_colors` with the information required by the next operation.
method_colors <- c(
  RRBLUP = "#3366CC",
  BayesA = "#DC3912",
  BayesB = "#FF9900",
...
```{r selected-mu-data}
# Prepare the objects required to perform the calculations at this stage.
burn_in_saved <-
...
for (trait_index in seq_along(traits)) {
  ...
  # Build `density_object` with the information required by the next operation.
...
# Build `correlation_plot` with the information required by the next operation.
correlation_plot <- ggplot(
  ...
  scale_fill_gradient2(
    ...
    high = "#3366CC",
    midpoint = 0,
    # Record the `limits` values that complete this structure.
    limits = c(-1, 1)
  ) +
  labs(
    x = NULL,
