...
A coluna de bloco (`BL`) não integra mais o modelo estatístico e é removida
imediatamente após a leitura dos dados fenotípicos. A família (`FAM`) é mantida
como a única covariável categórica para uso posterior como efeito fixo.
...
A coluna `BL` é descartada no momento da leitura porque bloco não participa
mais dos modelos. Em seguida, são mostradas amostras simples das duas fontes
para que o leitor visualize diretamente a estrutura dos dados utilizados.
...
# Amostra dos dados genotípicos lidos.
genes[1:6, 1:6]

# Amostra dos dados fenotípicos após a remoção de BL.
head(pheno_raw)
...
data.frame(
  ...
  kable(caption = "Dimensões dos dados após a leitura")
```
...
Dez características são utilizadas na análise. A família (`FAM`) é mantida
como variável categórica e será o único efeito fixo além do intercepto dos
modelos.
...
if (!all(identifier_integrity_checks)) {
  stop(
    "Falha no contrato de identificadores/alinhamento: ",
    paste(
...
kable(
  ...
  digits = 3,
  caption = "Resumo da distribuição da taxa de chamada SSR por indivíduo"
)
```
...
etapas seguintes. Os metadados contêm apenas o identificador do indivíduo e a
família utilizada como efeito fixo.
...
if (!all(preprocessing_integrity_checks)) {
  stop(
    "Falha no gate final de integridade do pré-processamento: ",
    paste(
...
de efeitos fixos contendo apenas família e define os cinco folds da validação
cruzada.
