...
O fluxo do módulo segue a sequência:

**ler as fontes → recuperar identificadores → organizar os SSR → alinhar os fenótipos → avaliar a qualidade genotípica → caracterizar os fenótipos → exportar os objetos validados.**

A família (`FAM`) é preservada porque será utilizada tanto na matriz de efeitos fixos quanto na estratificação dos folds de validação cruzada no Módulo 02. Nenhum indivíduo é removido neste módulo.
...
A preparação reúne apenas os recursos necessários para leitura, organização, visualização e exportação dos dados. As decisões metodológicas são apresentadas nas seções em que efetivamente são aplicadas, mantendo o fluxo do módulo rastreável.
...
### 2.2 Contrato do pré-processamento

O módulo preserva todos os indivíduos alinhados entre as fontes. O controle de qualidade atua sobre os locos SSR, `FAM` permanece como informação categórica para as etapas seguintes e as observações fenotípicas distantes da média são apenas sinalizadas para inspeção. Nenhum desses diagnósticos implica remoção automática de indivíduos ou transformação permanente dos fenótipos.

A saída principal é `preprocessing_object.rds`, acompanhada por arquivos CSV de diagnóstico. Essa interface permite que o Módulo 02 reutilize diretamente os objetos produzidos aqui.

## 3. Leitura e auditoria inicial das fontes
...
Os dados fenotípicos estão na aba `Médias_IND_artigo`, que já possui nomes de colunas. Essa fonte contém os identificadores dos indivíduos, a família e as dez características avaliadas.
...
A tabela fenotípica é inspecionada imediatamente após a importação:
...
A prévia mostra a estrutura da planilha fenotípica com `IND`, `FAM` e as dez características avaliadas ao longo da análise.

### 3.3 Conferência estrutural inicial

Antes de organizar e alinhar os dados, as dimensões das três estruturas importadas são comparadas. Essa verificação é estrutural: `Genes` contém os códigos genotípicos, `Genealex` preserva os metadados do formato GenAlEx e `pheno_raw` preserva a tabela fenotípica importada.
...
### 4.2 Construção da matriz SSR e representação de dados ausentes
...
Esta etapa transforma a tabela fenotípica bruta em objetos alinhados à matriz SSR. O processo é apresentado em três partes: definição das características e de seu dicionário, verificação da correspondência entre os indivíduos e, por fim, alinhamento dos fenótipos seguido da construção dos metadados e da matriz fenotípica.

### 5.1 Características e dicionário
...
### 5.2 Correspondência entre indivíduos

Antes de reordenar qualquer linha, definimos as colunas necessárias para a análise. Em seguida, verificamos os identificadores fenotípicos e comparamos o conjunto de plantas das duas fontes.

```{r correspondencia-individuos}
# Define as colunas necessárias para a análise.
...
### 5.3 Alinhamento e objetos analíticos
...
## 6. Controle de qualidade e completude genotípica
...
### 6.2 Resultado e visualização do controle de qualidade
...
### 6.3 Completude genotípica por indivíduo
...
### 7.2 Observações fenotípicas distantes da média
...
### 8.1 Objeto principal do pré-processamento
...
## 9. Síntese e transição para o Módulo 02

### 9.1 Inventário final
