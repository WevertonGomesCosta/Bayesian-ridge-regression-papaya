...
- arquivos temporários das cadeias do BGLR;
- uma linha no manifesto das execuções.

Uma execução é considerada concluída somente quando seu resultado RDS compacto
existe. Quando o módulo é reconstruído, as execuções concluídas são ignoradas e
apenas as ausentes são ajustadas. Isso fornece pontos de retomada
determinísticos sem reconstruir as matrizes ou alterar silenciosamente o
contrato dos modelos.

Os arquivos temporários do BGLR são lidos imediatamente após cada ajuste. O
BGLR grava amostras reduzidas pelo thinning durante o burn-in e durante a
amostragem posterior; por isso, a parte correspondente ao burn-in é removida
antes do cálculo dos diagnósticos de cadeia única. Depois, os arquivos brutos
são excluídos. Permanecem somente resumos posteriores compactos, predições,
valores genômicos, efeitos, diagnósticos e métricas.
...
As cadeias temporárias mantidas para o cálculo dos diagnósticos são:
