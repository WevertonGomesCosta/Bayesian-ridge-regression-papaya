...
The example table lets the reader follow each filter in the rule: competitiveness by correlation/RMSE, DIC proximity, chain-review status, and automatic eligibility. `Automatic_candidate = TRUE` requires all of these operational conditions to be satisfied.
...
configurable tutorial rule, not an estimated optimal selection intensity. With 150 available candidates, 20% corresponds to 30 individuals. For traits whose objective is to decrease, the score is the negative genomic value.
...
The final chunk writes individual CSV files for inspection of the main derivations and a consolidated `module04_results.rds` for programmatic use by Module 05. These files contain results derived from Module 03 outputs; no genomic model is refitted at this stage.
