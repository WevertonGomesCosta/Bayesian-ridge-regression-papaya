...
data.frame(
  ...
  # Organize the `Traits` component of this structure.
  Traits = c(
...
model_comparison <- pooled_metrics |>
  ...
  mutate(
    ...
    # Define the `ER` values used in this structure.
    ER = max(Wprob) / Wprob
  ) |>
  ungroup() |>
  select(
...
selection_spec <- predictive_winners |>
  ...
  left_join(
    ...
    # Organize the `by` component of this structure.
...
# Define the path or location associated with `oof_direction_summary`.
...
# Summarize the results in `selection_cutoff_summary`.
...
# Create `selected_individuals_by_trait` for use in the following steps of this block.
