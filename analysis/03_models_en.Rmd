...
This module contains the complete cross-validated fitting of seven genomic
prediction methods for ten papaya traits. The full 350-fit analysis is
implemented directly in this workflowr document. Ordinary rendering reads and
presents the existing canonical object, whereas a complete model refit is
enabled only through the explicit execution gate described in Section 3. The
same five deterministic, family-stratified folds are used for every method and
trait.
...
The complete model-fitting code is integrated directly below. Ordinary workflowr builds do not execute the fitting chunk and therefore do not modify the canonical M03 outputs.

To deliberately reproduce all 350 fits from the English document, use:

```bash
PAPAYA_RUN_M03_FIT=en Rscript --vanilla -e 'workflowr::wflow_build("analysis/03_models_en.Rmd")'
```

Both language versions contain the same analytical fitting code. A complete refit must be triggered from only one language document at a time; the other document can then be rendered normally from the resulting canonical object.
...
if (!file.exists(MODELS_FILE)) {
  stop("Canonical M03 object not found. Refit this document with `PAPAYA_RUN_M03_FIT=en`.")
}
...
The complete `ETA` specifications and the fixed-lambda Bayesian Lasso contract are implemented directly in the fitting chunk in Section 3.
