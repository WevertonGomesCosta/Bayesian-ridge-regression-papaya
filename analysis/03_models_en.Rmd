...
THIN <- 4L
# Use R2 = 0.5 as BGLR's global prior-allocation parameter.
R2_PRIOR <- 0.5
...
The table makes explicit the calibration leading to the fixed lambda. `MSx` summarizes the quadratic scale of `M`, `Global R2` is the value passed to BGLR, and `BL R2` is the share used in this calibration because the predictor contains two linear terms. The resulting `Lambda0` is then reused without trait- or fold-specific tuning.
