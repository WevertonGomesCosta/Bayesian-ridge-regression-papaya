...
data.frame(
  Item = c(
    ...
    "Características com evidência preditiva suportada"
  ),
  Valor = c(
    length(traits),
    length(methods),
...
A seleção utiliza as características com correlação OOF positiva e
`p < 0,05`. A proporção utilizada é de
`r sprintf("%.0f%%", 100 * selection_proportion)` dos indivíduos por
característica, respeitando a direção de melhoramento definida no Módulo 04.
...
kable(
  ...
  digits = 2,
  caption = "Proporção e número de indivíduos selecionados por característica"
)
```

Os primeiros indivíduos de cada ranking permitem visualizar diretamente a
seleção produzida.
...
selected_individuals_by_trait |>
  ...
  kable(
    digits = 5,
    caption = "Cinco primeiros indivíduos selecionados por característica"
  )
```
...
A recorrência resume em quantas características suportadas cada indivíduo
aparece entre os 20% mais favoráveis. `N_traits`, frequência de seleção e ranks
