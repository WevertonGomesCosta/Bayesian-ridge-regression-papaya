...
# Constrói `method_labels` com a informação necessária para a operação seguinte.
method_labels <- c(
  RRBLUP = "RR-BLUP/BRR",
  BayesA = "BayesA",
  BayesB = "BayesB",
...
# Constrói `method_colors` com a informação necessária para a operação seguinte.
method_colors <- c(
  RRBLUP = "#3366CC",
  BayesA = "#DC3912",
  BayesB = "#FF9900",
...
final_results_summary <- model_selection |>
  mutate(
    ...
    Selection_status_label = case_when(
      # Define os valores de `Selection_status` usados nesta estrutura.
  ...
  transmute(
    ...
    # Define os valores de `DIC_winner` usados nesta estrutura.
...
for (trait_index in seq_along(traits)) {
  ...
  # Constrói `density_object` com a informação necessária para a operação seguinte.
...
# Constrói `correlation_plot` com a informação necessária para a operação seguinte.
correlation_plot <- ggplot(
  ...
  scale_fill_gradient2(
    ...
    high = "#3366CC",
    midpoint = 0,
    # Registra os valores de `limits` que completam esta estrutura.
    limits = c(-1, 1)
  ) +
  labs(
    x = NULL,
