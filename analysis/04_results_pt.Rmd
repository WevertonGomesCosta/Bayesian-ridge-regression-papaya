...
Por fim, são reunidos o método selecionado, os vencedores preditivo e por DIC e
...
model_selection |>
  select(
    ...
    RRBLUP_preference_applied,
    Selection_status, OOF_predictive_direction
  ) |>
  kable(
    digits = 5,
...
selected_oof_genomic_values <- predictions |>
  inner_join(
    model_selection |>
      select(Trait, Selected_method, OOF_predictive_direction),
    by = "Trait"
  ) |>
...
A seleção é restrita às características para as quais o método selecionado
apresenta correlação OOF positiva. Os p-valores nominais permanecem descritivos
e não determinam a elegibilidade. Em cada característica elegível, são
selecionados os 20% de indivíduos com maior escore orientado; essa proporção de
20% é uma regra configurável do tutorial, e não uma intensidade ótima estimada.
Para características cujo objetivo é diminuir, o escore é o negativo do valor
genômico.
...
selection_eligible_traits <- model_selection |>
  filter(OOF_predictive_direction == "positive") |>
  pull(Trait)

selection_sizes <- genomic_ranking |>
  filter(Trait %in% selection_eligible_traits) |>
...
kable(
  selection_sizes,
  digits = 2,
  caption = "Número de indivíduos selecionados por característica elegível para seleção"
)
```
...
selected_individuals_by_trait |>
  ...
  kable(
    digits = 5,
    caption = "Cinco primeiros indivíduos do ranking em cada característica elegível para seleção"
  )
```
...
seleção, em quantas cada indivíduo aparece entre os 20% mais favoráveis. Não são usados pesos e não é
...
multitrait_recurrence <- matrices_object$metadata |>
  ...
  mutate(
    ...
    Selection_frequency = N_traits / length(selection_eligible_traits),
...
kable(
  ...
  digits = 4,
  caption = "Distribuição da recorrência entre características elegíveis para seleção"
)
```
...
saveRDS(
  list(
    ...
    selection_proportion = SELECTION_PROPORTION,
    selection_eligible_traits = selection_eligible_traits,
