...
predictive_plot <- model_comparison |>
  ...
  labs(
    ...
    subtitle = paste0(
      "A linha pontilhada indica r OOF = ",
      sprintf("%.2f", OOF_R_SELECTION_THRESHOLD),
      "; a elegibilidade exige r superior a esse valor."
    )
  ) +
  project_theme +
...
em que \(s_y\) é o desvio-padrão fenotípico observado. O primeiro critério exige correlação OOF observado × predito **positiva** e superior a 0,30. Correlações OOF nulas ou negativas não são elegíveis, mesmo quando apresentam grande magnitude absoluta, porque não preservam o ordenamento preditivo desejado.
...
O limiar de elegibilidade exige correlação preditiva OOF positiva: \(r_{OOF} > 0{,}30\). A decisão é apresentada do maior para o menor `r`. A matriz de correlações entre GEBVs OOF das dez características permanece exclusivamente descritiva e não usa esse limiar.
...
tabela_html(
  ...
  caption = "Elegibilidade para seleção genômica, ordenada pela correlação OOF",
  col.names = c(
...
Com \(r > 0{,}30\) e `NRMSE < 1`, permanecem elegíveis **MMCOM**, **CF**, **PRODCOM** e **DF**. DCI falha apenas no limiar positivo de correlação OOF. FFC, SS, FFP, PRODEF e EP apresentam \(r \le 0{,}30\) e também `NRMSE >= 1` nos valores não arredondados. A tabela explicita separadamente correlação, sinal, erro e motivo da decisão.
...
O módulo grava exatamente onze tabelas canônicas em formato legível por máquina diretamente em `output/results/m04/`. Esses arquivos formam o contrato público de resultados rastreado pelo Git. Tabelas detalhadas adicionais e o objeto RDS consolidado são armazenados em `output/results/m04/details/` e permanecem como artefatos gerados localmente e não rastreados pelo Git.

As exportações públicas são independentes do idioma: as fontes PT e EN utilizam os mesmos nomes de objetos analíticos e arquivos, enquanto os rótulos traduzidos permanecem restritos à apresentação nas páginas HTML.
...
# Mantém tabelas detalhadas e não públicas separadas do contrato público rastreado pelo Git.
write.csv(
  oof_genomic_values,
...
tabela_html(
  ...
  caption = "Saídas públicas canônicas gravadas pelo Módulo 04",
  col.names = c("Arquivo", "Objeto analítico")
)
```
